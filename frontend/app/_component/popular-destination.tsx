          ...
          {destinations?.slice(0, 4).map((destination: destinationProps) => {
            return (
              <Link
                ...
                key={destination.id}
                className="border flex flex-col justify-start items-stretch shadow-sm rounded-lg overflow-hidden"
              >
                <Image
                  src={
                    destination?.cover ||
                ...
                <div className="p-4 gap-2 flex flex-col justify-start items-stretch">
                  ...
                  <div className="flex text-primary justify-start items-center gap-2">
                    <Pin className="w-4 h-4" />
                    <p className="line-clamp-1 font-bold text-sm">{destination?.district?.name}</p>
                  </div>
